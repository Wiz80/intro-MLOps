...
def save_simple_metrics_score(train_score: float, test_score: float, validation_score: float, model: Pipeline) -> None:
...
def get_model_performance_test_set(y_test: pd.Series, y_test_pred: pd.Series):
    fig, ax = plt.subplots()
    ...
    ax.set_title('R2 of model production')

    fig.savefig('predicted_behavior.png')